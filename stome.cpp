...
/**
   ...
   Notes:
     ...
     (2) Move constructor (new in C++11).  Essential for good performance of some algorithms like std::sort.  After moved from an
         object must still support assignment (both move and traditional).

     (3) The default, no-arg, constructor is required for arrays & preallocated vectors -- make it efficient!

     (4) Destructor.  Required for STL containers.  Must never throw exceptions.

     (5) Assignment operator.  Required for STL containers. Must deal well with self assignment.

     (6) The move assignment (new in C++11).  See notes item 2 & 5.

     (7) "Less Than" operator used for things like std::sort, std::map, and std::lower_bound.

     (8) == operator used for things like std::find

     (9) The 'setter/getter method' is a common idiom -- more popular in Java than C++.  The existence of several setter/getter
         methods in an object can be a sign of a poor design.
    ...
    (14) A 'to_s' member is not required.  The name is not standardized ('toString' is another common name).  This is a more common
         idiom in other languages (Java & Ruby).  In C++ a common, alternative, idiom is to use the << operator with a string stream.
